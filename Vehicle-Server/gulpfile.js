...
// Transfers data
gulp.task("data", () => {
  return gulp.src(["./src/**/*.csv"]).pipe(gulp.dest("./dist/tsc"));
});
...
gulp.task(
  "build",
  gulp.series("clean", "index", "icon", "data", "tsc", "bundle")
);
...
gulp.task("heroku-copy-root", () => {
  return gulp
    .src([
      ...
      // "./dist/tsc/server/data.js",
    ])
    .pipe(gulp.dest("./deploy"));
});

// // Heroku-copy-root-to-dist
// gulp.task("heroku-copy-root-to-dist", () => {
//   return gulp
//     .src(["./dist/tsc/client/shared/add.js", "./dist/add.html"])
...
gulp.task(
  "dev",
  gulp.series("build", gulp.parallel("watch-html", "watch-tsc", "tsc-w"))
);
...
gulp.task(
  ...
  gulp.series(
    "build",
    gulp.parallel("watch-html", "watch-tsc", "tsc-w", "express")
  )
);
